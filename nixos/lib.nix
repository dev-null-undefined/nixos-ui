let
  ...
  inherit (builtins) isString filter mapAttrs;
  inherit (lib) optionals filterAttrs;
  ...
  normalizeType = submoduleVisible: ty: let
    ...
    ty' =
      ...
      or {name = "any";};
  in
    assert ty._type or null == "option-type"; ty';
  ...
  normalizeOptions = opt: let
    ...
    opt' = {
      ...
      example =
        if opt ? example
        then renderOptionValue opt.example
        else null;
      default =
        if opt ? default
        then renderOptionValue (opt.defaultText or opt.default)
        else null;
      relatedPackages =
